...
1. Calc: Julia計算部分作成
    1. 期待リターン: 重みつき和（R = 重み * 期待リターン = Σ(wi * Ri)）✅
    1. リスク: 重みつき和の分散（Var(R) = Var（重み * 期待リターン）=Var(Σ(wi * Ri)) = ΣΣ(wi * wj * Cov(Ri, Rj))）✅
    1. ※共分散は事前に準備する
    1. ※共分散: Cov(Ri, Rj) = Σij = ρij × σi × σj = 相関係数 × ボラティリティ × ボラティリティ
...
## Day4
1. FE: Rechartによる3シナリオにおける期待リターンとリスクの可視化✅
1. 発表の筋を決める✅
1. 
1. Calc: シャープレイシオ（期待リターン / リスク）ランキングの計算式作成
